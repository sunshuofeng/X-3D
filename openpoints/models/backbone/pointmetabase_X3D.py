...
import copy
from .X_3D_utils.explict_structure import *
from .X_3D_utils.neighbor_context import *
from .X_3D_utils.structure_kernel import *
...
class LocalAggregation(nn.Module):
    ...
    def forward(self, pf, pe) -> torch.Tensor:
        ...
        f = self.convs1(f)
        # grouping
        group_idx=self.grouper.forward_idx(p,p,f)
        dp, fj = self.grouper(p, p, f,idx=group_idx)
        # pe + fj 
        f = pe + fj
        f = self.pool(f)
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        self.x3d=x3d
        if x3d:
            es_type=kwargs.get('es_type','pca')
            structure_kernel_args=kwargs.get('structure_kernel_args')
            ES=LOCAL_DICT[es_type]
            self.ES=ES()
            es_channel=self.ES.outchannel
            pe_mul=nn.Sequential(*convs2)
            self.dynamic_latent=X3D_Model(es_channel,channels[-1],pe_mul,structure_kernel_args)
            self.dynamic_dilated=None
            if ncp!='false':
                self.dynamic_dilated=Neighbor_Context(channels[-1])
    ...
    def forward(self, pf_pe):
        ...
        if self.is_head:
            ...
            group_idx=self.grouper.forward_idx(new_p,p,f)
            dp, fj = self.grouper(new_p, p, f,group_idx)

            if self.x3d:
                es=self.ES(dp.permute(0,2,3,1).contiguous(),new_p,group_idx)
                f=self.dynamic_latent(es,dp,group_idx,fj)
            ...
            if self.x3d and self.dynamic_dilated is not None:
                
                f=self.dynamic_dilated(f,group_idx,idx,N)
...
class PointMetaBaseEncoder_X3D(nn.Module):
    ...
    def __init__(self,
        ...
        x3d_layers=kwargs.get('x3d_layers',[False,False,False,False,False])
        self.x3d_layers=x3d_layers
        ncp=kwargs.get('NCP','false')
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                ...
                is_head=i == 0 and strides[i] == 1,x3d=self.x3d_layers[i],ncp=ncp,
            ))
            if i == 0:
    ...
    def _make_enc(self, block, channels, blocks, stride, group_args, is_head=False,x3d=False,ncp=False):
        layers = []
        ...
        layers.append(SetAbstraction(self.in_channels, channels,
                                     ...
                                     is_head=is_head, use_res=self.sa_use_res,x3d=x3d,ncp=ncp, **self.aggr_args 
                                     ))
